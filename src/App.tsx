...
import { timetableData, todayIndex } from './data/timetable';
import { FeedbackBox } from './components/FeedbackBox';
import { Footer } from './components/Footer';
import { useNowIST, getNowIST } from './utils/timeUtils';
import { motion } from 'framer-motion';
import clsx from 'clsx';

function App() {
  // ── Live clock (updates every 30 s) ────────────────────────────────────────
  const nowMinutes = useNowIST();

  // ── Today index — recomputed whenever the live clock ticks ─────────────────
  // todayIndex() returns -1 on Sunday, 0–5 Mon–Sat
  const today = useMemo(() => todayIndex(), [nowMinutes]);

  const [selectedDay, setSelectedDay] = useState(() =>
    today >= 0 ? today : 0   // default to Monday on Sunday
  );
  ...
  // ── Sunday modal ───────────────────────────────────────────────────────────
  const isSunday = getNowIST().getDay() === 0;
  const [showSundayMessage, setShowSundayMessage] = useState(
    isSunday && sessionStorage.getItem('sundayMessageShown') !== 'true'
  );
  const [showTimetable, setShowTimetable] = useState(!isSunday);

  useEffect(() => {
    if (isSunday && sessionStorage.getItem('sundayMessageShown') !== 'true') {
      ...
      {showSundayMessage && (
        ...
        >
          ...
          <p className="text-gray-100 text-lg font-medium mb-6">
            No classes today (Sunday). Enjoy your weekend!
          </p>
          <button
      ...
      <div className="relative z-10 flex-1 flex flex-col max-w-7xl mx-auto w-full px-4 sm:px-6 py-4 sm:py-8">
          ...
          <div className="flex items-center justify-between mb-6 sm:mb-8">
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div>
                ...
                <p className="text-gray-400 text-[0.7rem] sm:text-sm font-medium">
                  Timetable
                </p>
              </div>
            </div>
            ...
            >
              <motion.div
                initial={false}
                animate={{ y: isDarkMode ? 40 : 0, opacity: isDarkMode ? 0 : 1, rotate: isDarkMode ? 45 : 0 }}
              ...
              <motion.div
                initial={false}
                animate={{ y: isDarkMode ? 0 : -40, opacity: isDarkMode ? 1 : 0, rotate: isDarkMode ? 0 : -45 }}
        ...
        <nav className="flex gap-2 sm:gap-3 mb-6 sm:mb-8 overflow-x-auto pb-4 scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0">
          {timetableData.map((schedule, index) => {
            ...
            // today >= 0 means Mon–Sat; match index to today
            const isToday = today >= 0 && index === today;
            ...
            return (
              ...
              >
                ...
                <span className="relative z-10 flex items-center gap-2">
                  {schedule.day}
                  {/* Green live-dot — only on the actual current day */}
                  {isToday && (
          ...
          {showTimetable && (
            <div className="flex-1 overflow-y-auto scrollbar-hide pr-1 sm:pr-2">
              <DaySchedule
                ...
                isToday={today >= 0 && selectedDay === today}
